...
<div class="subnav">
		...
		<tr>
			<th>会员级别：</th>
			<td>
			<input type="radio" name="data[level]" value="0" <?php if (!isset($data['level']) || $data['level']==0) { ?>checked<?php } ?>> 权限开放 &nbsp;
			<?php if (is_array($this->level_arr)) foreach ($this->level_arr  as $key=>$t) { ?>
				<input type="radio" <?php if (isset($data['level']) && $data['level']==$key) { ?>checked<?php } ?> value="<?php echo $key; ?>" name="data[level]" > <?php echo $t; ?> &nbsp;
			<?php } ?><div class="onShow">默认开放！ </div>
			</td>
		</tr>
		<tr>
			<th>&nbsp;</th>
