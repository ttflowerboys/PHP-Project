...
class c_users extends SAdmin{
	...
	public function add(){
		...
		TableRow(array('<b>用户类型:</b>', '<select name="usertype"><option value="1" '.Iif($user['usertype']==1,'selected','').'>系统管理员</option><option value="3" '.Iif($user['usertype']==3,'selected','').'>客户</option><option value="2" '.Iif($user['usertype']==2,'selected','').'>调查员</option></select>'));
		if($userid){
