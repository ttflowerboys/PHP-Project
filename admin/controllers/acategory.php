...
class c_acategory extends SAdmin{
	...
	private function GetOptions($categories, $currentid = 0, $selectedid = 0, $parentid = 0, $sublevelmarker = ''){
		...
		foreach($categories as $value){
			if($parentid == $value['p_id'] AND $value['cat_id'] != $currentid){
				$sReturn .= '<option '. Iif(!$parentid, 'style="color:#cc4911;font-weight:bold;"') . ' value="' . $value['cat_id'] . '" ' . Iif($selectedid == $value['cat_id'], 'SELECTED', '') . '>' . $sublevelmarker . $value['name'] . '- (' . $value['counts'] . ')</option>';
	...
	public function save(){
		...
		if(isset($errors)){
			...
			if($cat_id){
				...
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "acat (p_id, is_show, show_sub, name, keywords, desc_cn) VALUES ('$p_id', '$is_show', '$show_sub', '$name', '$keywords', '$desc_cn') ");
	...
	public function index(){
		...
		$getcategories = $this->db->query("SELECT cat_id, p_id, sort, is_show, show_sub, name, counts FROM " . TABLE_PREFIX . "acat ORDER BY sort");
	...
	private function ShowCategories ($parentid = 0, $sublevelmarker = '') {
		...
		foreach($allcategories as $key => $value){
			if($parentid == $value){
				TableRow(array($sublevelmarker .'<input type="hidden" name="cat_ids[]" value="' . $key . '" /><input type="text" name="sorts[]" value="' . $this->categories[$key]['sort'] . '" size="4">&nbsp;&nbsp;<input type="text" name="names[]" value="' . $this->categories[$key]['name'] . '" size="22" />',
