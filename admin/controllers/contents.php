...
class c_contents extends SAdmin{
	...
	public function index(){
		...
		TableRow(array('调用ID', '中文名称', '日期', '<input type="checkbox" id="checkAll" for="deletec_ids[]"> <label for="checkAll">删除</label>'), 'tr0');
	...
	public function save(){
		...
		if($c_id){
			...
			$getmax = $this->db->getOne("SELECT MAX(r_id) as max FROM " . TABLE_PREFIX . "content");
			$r_id = $getmax['max'] + 1;

			$this->db->exe("INSERT INTO " . TABLE_PREFIX . "content (title, keywords, content, created, r_id) VALUES ('$title', '$keywords', '$content', '$time', '$r_id') ");
