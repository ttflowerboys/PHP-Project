...
class c_articles extends SAdmin{
	...
	public function index(){
		...
		$newcategories = array();
		$getcategories = $this->db->query("SELECT cat_id, p_id, name, counts FROM " . TABLE_PREFIX . "acat ORDER BY sort");
		...
		$getarticles = $this->db->query("SELECT a_id, sort, cat_id, is_show, is_best, userid, username, title, clicks, created FROM " . TABLE_PREFIX . "article " . $Where . " ORDER BY is_show ASC, sort DESC LIMIT $start, $NumPerPage");
		...
		if($maxrows['value'] < 1){
			...
			while($article = $this->db->fetch($getarticles)){
				...
				TableRow(array('<input type="hidden" name="a_ids[]" value="'.$article['a_id'].'" /><input type="text" name="sorts[]" value="' . $article['sort'] . '" size="4" />',
					'<a href="'.BURL('articles/edit?a_id='.$article['a_id']).'" >' . $title . '</a>',
	...
	public function add(){
		...
		TableRow(array('<B>正文:</B><BR><span class=light>文章的详细内容.</span>', '
			<div class="ok_tab">
				<div class="ok_tabheader">
					<ul id="tabContent-li-ok_tabOn-">
						...
						<li><a href="javascript:void(0)" title="缩略图片" rel="2" hidefocus="true">缩略图片</a></li>
					</ul>
				</div>
	...
	public function save(){
		...
		if(isset($errors)){
			...
			if($a_id){
				...
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "article (cat_id, is_show, is_best, userid, username, title, content,content_en, keywords, clicks, created) VALUES ('$cat_id', '$is_show', '$is_best', '$userid', '$username', '$title', '$content','$content_en',  '$keywords',  '0', '$time') ");
	...
	private function GetSelect($selectedid =0, $selectname = 'cat_id'){
		...
		$categories = $this->db->getAll("SELECT cat_id, p_id, name, counts  FROM " . TABLE_PREFIX . "acat ORDER BY sort");
	...
	private function GetOptions($categories, $selectedid = 0, $p_id = 0, $sublevelmarker = ''){
		...
		foreach($categories as $value){
			if($p_id == $value['p_id']){
				$sReturn .= '<option '. Iif(!$p_id, 'style="color:#cc4911;font-weight:bold;"') . ' value="' . $value['cat_id'] . '" ' . Iif($selectedid == $value['cat_id'], 'SELECTED', '') . '>' . $sublevelmarker . $value['name'] . '- (' . $value['counts'] . ')</option>';
