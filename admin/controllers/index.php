...
class c_index extends SAdmin{
    ...
    function index($Path){
		...
		TableRow(array('3)', '新闻总数: <font class='.Iif($basedata['totalnews'], 'greenb').'>'.$basedata['totalnews'].'</font>', '未发布新闻数: <font class='.Iif($basedata['noactivenews'], 'redb').'>'.$basedata['noactivenews'].'</font>', '<a class="link-btn" href="'.BURL('news').'">管理新闻</a>'));

		TableRow(array('4)', '常态内容数: <font class='.Iif($basedata['totalcontents'], 'greenb').'>'.$basedata['totalcontents'].'</font>', '&nbsp;', '<a class="link-btn" href="'.BURL('contents').'">常态内容</a>'));
