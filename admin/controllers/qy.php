...
class c_qy extends SAdmin{
	...
	public function save(){
		...
		if(strlen($username) == 0){
			...
			$errors[] = '用户名存在非法字符!';
		}elseif($this->db->getOne("SELECT userid FROM " . TABLE_PREFIX . "admin WHERE username = '$username' AND usertype='$usertype' AND userid != '$userid'")){
			$errors[] = '用户名已存在!';
		}
		...
		if(isset($errors)){
			...
			if($userid){
				$this->db->exe("UPDATE " . TABLE_PREFIX . "admin SET username    = '$username', usertype='$usertype',
				".Iif($userid != $this->admin->data['userid'], "activated = '$activated',")."
				".Iif($password, "password = '" . md5($password) . "',")."
				WHERE userid      = '$userid'");

			}else{
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "admin (username, password, joindate, joinip,usertype) VALUES ('$username', '".md5($password)."',  '".time()."', '".GetIP()."', '$usertype')");
	...
	public function add(){
		...
		TableRow(array('<b>用户类型:</b>', '<select name="usertype"><option value="1" '.Iif($user['usertype']==1,'selected','').'>系统管理员</option><option value="2" '.Iif($user['usertype']==2,'selected','').'>普通用户</option></select>'));
		if($userid){
	...
	public function index(){
		...
		$getusers = $this->db->query("SELECT * FROM " . TABLE_PREFIX . "admin ".$searchsql." ORDER BY userid ASC, userid DESC LIMIT $start,$NumPerPage");
		...
		TableRow(array('ID', '用户名', '登录次数', '注册日期', '注册IP', '最后登陆', '最后IP', '用户类型','<input type="checkbox" id="checkAll" for="deleteuserids[]"> <label for="checkAll">删除</label>'), 'tr0');

		if($maxrows['value'] < 1){
			TableRow('<center><BR><font class=redb>未搜索到任何用户!</font><BR><BR></center>');
		}else{
			while($user = $this->db->fetch($getusers)){
				TableRow(array($user['userid'], '<input type="hidden" name="updateuserids[]" value="'.$user['userid'].'" /><a href="'.BURL('users/edit?userid='.$user['userid']).'">'. $user['username'] .'</a>',
				$user['loginnum'],
				DisplayDate($user['joindate']),
				$user['joinip'],
				Iif($user['lastdate'] == 0, '<span class="orange">从未登陆</span>', DisplayDate($user['lastdate'])),
				$user['lastip'],
				Iif($user['usertype'] == 1, '系统管理员','普通用户'),
