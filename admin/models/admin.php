...
class admin{
	...
	private function auth($ajax){
		...
		if($sessionid AND IsAlnum($sessionid)){//登录成功验证cookie授权
			$sql = "SELECT s.sessionid, u.userid, u.username FROM " . TABLE_PREFIX . "sessions s
...
$(document).ready(function() {     
   ...
   private function check(){
		...
		if(!strlen($username) OR !strlen($password)){
			...
			$user = $this->db->getOne("SELECT u.userid FROM " . TABLE_PREFIX . "admin u WHERE u.username = '$username' AND u.password = '$password'  AND u.usertype=1");
