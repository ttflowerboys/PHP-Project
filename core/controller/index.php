...
class index extends Base {
	...
	public function formAction() {
	    ...
	    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    		...
    		if (!empty($form_model['setting']['form']['code']) && !$this->checkCode($this->post('code'))) $this->tp_error('验证码不正确');
			if (!empty($form_model['setting']['form']['post']) && !$this->member_info) $this->tp_error('只允许会员提交，请注册会员后提交');
			if (!empty($form_model['setting']['form']['time'])){
		        ...
		        if (time() - $ipdata['time'] < $time) $this->tp_error('同一IP在'. $form_model['setting']['form']['time'] .'分钟内不能重复提交');
			}
			if (!empty($form_model['setting']['form']['num']) && !empty($form_model['setting']['form']['post']) && $this->member_info ) {
				...
				if (!empty($form_model['joinid'])) $this->db->where('cid=?', $cid);
		        if ($this->db->getOne('','','id')) $this->tp_error('您已经提交过了，不能重复提交');
			}
			$data = $this->post('data');
	        unset($data['id']);
            ...
            if ($this->db->setTableName($form_model['tablename'])->insert($data,true)) {
                ...
                $data['status'] ? $this->tp_success('提交成功') : $this->tp_error('提交成功，等待审核');
			} else {
			    $this->tp_error('提交失败');
			}
		}
