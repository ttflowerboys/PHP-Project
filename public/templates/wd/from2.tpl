       ...
       <div class="clearfix">
	        <div class="w-md-2">
	          <div class="addr">
	          	...
	          	<div class="info">
	          		<p><strong>店面编码：</strong>{$dp_info.dp_no}</p>
	          		<p><strong>营销区域：</strong>{echo areaType($dp_info.yx_catid)} - {echo areaType($dp_info.qy_catid)}</p>          		
	          		<p><strong>店铺名称：</strong>{$dp_info.name}</p>
	          		<p><strong>店面地址：</strong>{$dp_info.address}</p>
	          		<p><strong>检测日期：</strong>{echo DisplayDate($dp_info.zftime)} {$dp_info.zfintime} ~ {$dp_info.zfouttime}</p>
	          	</div>
	          </div> <!-- addr end -->				
	        </div>
			<div class="w-md-2">
				<div class="defen">
					<div class="defenItems"><i class="icon-man"></i><b>{$dp_info.manfen}</b><span>男经纪人得分</span></div>
					<div class="defenItems"><i class="icon-woman"></i><b>{$dp_info.womanfen}</b><span>女经纪人得分</span></div>
					<div class="defenItems"><i class="icon-store"></i><b>{$dp_info.storefen}</b><span>店面环境得分</span></div>
					<div class="defenItems"><i class="icon-main"></i><b>{$dp_info.totalfen}</b><span>店铺总体得分</span></div>
				</div>
			</div>
	    </div> <!-- 店铺信息 end -->
	    ...
	    <div class="clearfix">
	    	<div class="w-md-2">  
	        	<div class="User">
	        	  ...
	        	  {if $rs.type==0}
	        	  <div class="w-md-2">
	              	<a class="info" href="{PURL('jjr?id='. $rs.id)}" target="_blank">
	              	    <i class="icon-man"></i>
...
<div class="spread2" id="manBox">
    <form class="ajaxForm" action="{PURL('from2/jjrAddDo')}" enctype="multipart/form-data" method="post">
	<div class="upBox">
		...
		<hr class="hr">
		<input type="hidden" name="dp_id" value="{$dp_info.id}">
