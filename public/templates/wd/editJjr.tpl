...
<div class="spread">
    ...
    {else} 
    	  <div class="w-md-4">
    	  	<a class="info" href="{PURL('record?id=' . $cur_jjr.dp_id.'&show=2&jjr='.$cur_jjr.id)}" title="点我查看～">
          	    {if $cur_jjr.type==0}<i class="icon-man"></i>{else}<i class="icon-woman"></i>{/if}
          		<div class="userInfo"><strong>姓名：</strong>{$cur_jjr.username}<span class="t-green">（{$cur_jjr.level}）</span><br>电话：<span class="t-green">{$cur_jjr.mobile}</span> <br>工号：<span class="t-green">{$cur_jjr.no}</span></div>
          	</a>
          </div>
    {/if}
    </div>
...
<input id="fileupload" type="file" name="files[]" multiple="" style="display:none;">
<div class="spread2">
  <form class="ajaxForm" action="{PURL('jjr/update')}" enctype="multipart/form-data" method="post">
    <div class="upBox">
          ...
          <div class="w-md-85p userMain">
            <div class="w-md-4">
            	<div class="form-items">
	                <span class="input-group-addon">姓名</span><input type="text" class="input-text" value="{$cur_jjr.username}" name="username" required>
	            </div>
            </div>	            
            <div class="w-md-4">
            	<div class="form-items">
	                <span class="input-group-addon">电话</span><input type="text" class="input-text"  value="{$cur_jjr.mobile}" name="mobile">      
	            </div>
            </div>
            <div class="w-md-4">
            	<div class="form-items">
	                <span class="input-group-addon">工号</span><input type="text" class="input-text"  value="{$cur_jjr.no}" name="no">
	            </div>
            </div>
            <div class="w-md-4">
            	<div class="form-items">
	                <span class="input-group-addon">级别</span><input type="text" class="input-text" value="{$cur_jjr.level}" name="level">
	            </div>
            </div>           
        </div>
	</div> <!-- end -->
	  <hr class="hr">
		...
		<div class="clearfix">
		  	...
		  	<div class="w-md-7_5p">
		  	  <textarea name="impressions" class="textarea input-textarea" placeholder="请描述该经纪人给您的第一印象如何？" >{$cur_jjr.impressions}</textarea>
		  ...
		  <div class="clearfix">
		  	<div class="w-md-4">
		  		<div class="FromHd"><span>举例说明</span><input type="text" name="descscore" class="input-text input-text-descscore" value="{$cur_jjr.descscore}"></div>
		  	</div>
		  	<div class="w-md-7_5p">
		  	  <textarea class="textarea input-textarea" name="exinfo" placeholder="那么请您描述此名经纪人哪些方面不符合商务人士的形象" >{$cur_jjr.exinfo}</textarea>
		...
		<hr class="hr">
		<input type="hidden" name="type" value="{$cur_jjr.type}">
		<input type="hidden" name="id" value="{$cur_jjr.id}">
